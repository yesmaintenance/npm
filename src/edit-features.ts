...
for (const repo of (await octokit.request(`GET /users/${user}/repos`)).data) {
	repos.push({
		owner: user,
		name: repo.name,
	});
...
for (const org of (await octokit.request(`GET /users/${user}/orgs`)).data) {
	...
	for (const repo of (await octokit.request(`GET /orgs/${org.login}/repos`))
		.data) {
		repos.push({
			owner: org.login,
			name: repo.name,
...
for (const repo of repos) {
	...
	try {
		...
		console.log(`Could not enable vulnerability-alerts for: ${repo.name}`);
	}
	...
	try {
		...
		console.log(`Could not update actions/permissions for: ${repo.name}`);
	}
	// end: actions/permissions
	...
	try {
		...
		console.log(`Updated actions/permissions/workflow for: ${repo.name}`);
	} catch (error) {
		console.log(
	...
	// start: starred
	try {
		await octokit.request(`PUT /user/starred/${repo.owner}/${repo.name}`);
